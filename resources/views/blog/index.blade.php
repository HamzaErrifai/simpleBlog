...
@section('content')
    ...
    @if (session()->has('message'))
        <div class="w-4/5 m-auto mt-10 pl-2">
            <p class="w-1/6 mb-4 text-gray-50 bg-green-500 rounded-2xl px-6 py-4">
                {{ session()->get('message') }}
            </p>
        </div>
    @endif
    ...
    @if (Auth::check())
        <div class="pt-15 w-4/5 m-auto">
            <a class="bg-blue-500 uppercase bg-transparent text-gray-100 text-xs font-extrabold py-3 px-5 rounded-3xl"
                href="/blog/create">
                Create a post
            </a>
        </div>
    ...
    @foreach ($posts as $post)
        <div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200">
            <div class="">
                <img src="{{ asset($post->image_path) }}" width="700" alt="">
            </div>
            <div class="">
